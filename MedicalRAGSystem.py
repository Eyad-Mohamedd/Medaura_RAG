...
Patient: {record.get('patient_type', 'Unknown')}

Symptoms Description: {record.get('user_symptom_description', '')}

Extracted Symptoms: {record.get('extracted_symptoms', '')}

Possible Conditions: {record.get('possible_conditions', '')}
...
User Query: {user_query}"""
    ...
    def _generate_answer_with_history(
        ...
        # [CHANGED] Added multilingual instruction at the top of the prompt:
        # the model now detects the language of the question and replies in the same language.
...
- Carefully match symptom SEVERITY.
...
1) One or two sentences describing the MOST LIKELY condition(s)
2) One sentence clearly stating the urgency level
3) One sentence recommending the most appropriate medical specialty

Choose the specialty based on the MOST LIKELY condition — not the most dangerous rare scenario.
"""
